...
export function fetchLinksFromHtml(app, microAppHead, htmlDom) {
  ...
  for (const [url] of linkEntries) {
    ...
    const newUrl = `${
      app.url.endsWith("/") ? app.url.substring(0, app.url.length - 1) : app.url
    }${url}`;
    console.log("fetch link:", newUrl);
    fetchLinkPromise.push(fetchSource(newUrl));
  }
...
export function fetchScriptsFromHtml(app, htmlDom) {
  ...
  for (const [url, info] of scriptEntries) {
    ...
    fetchScriptPromise.push(
      info.code ? Promise.resolve(info.code) : fetchSource(newUrl)
    );
  }
